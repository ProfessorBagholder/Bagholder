//! JSON read exactly: a value tree in which every number is the decimal its
//! digits spell, never a float.
//!
//! A source's reply and a record's payload are both JSON, and both are read
//! here. A number is kept as canonical decimal text worked out from its digits
//! as written (`1.10`, `1.1` and `11e-1` are all `1.1`), so no digit a source
//! sent is lost and reading it as a [`Dec`](crate::Dec) is exact or refused.
//! JSON that repeats a key within one object is refused: which value the source
//! meant cannot be known.
//!
//! [`Value::canonical`] writes the one text form the book keeps of a payload:
//!
...
pub struct JsonError {
    /// The byte of the text where reading stopped.
    pub at: usize,
    pub why: String,
}
...
impl fmt::Display for JsonError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "not JSON, at byte {}: {}", self.at, self.why)
    }
}
...
impl Value {
    ...
    /// What kind of value this is, in words, for a reader's mismatch.
...
/// One JSON value, and nothing but whitespace around it.
pub fn parse(json: &str) -> Result<Value, JsonError> {
...
impl Parser<'_> {
    fn err(&self, why: &str) -> JsonError {
        JsonError { at: self.i, why: why.to_string() }
    }
    ...
    fn object(&mut self, depth: usize) -> Result<Value, JsonError> {
        ...
        loop {
            ...
            if map.insert(key.clone(), v).is_some() {
                return Err(JsonError { at, why: format!("the key {key:?} appears twice in one object") });
            }
            self.ws();
            match self.peek() {
                Some(b',') => self.i += 1,
    ...
    fn string(&mut self) -> Result<String, JsonError> {
        ...
        loop {
            ...
            let run = std::str::from_utf8(&self.s[start..self.i]).map_err(|_| JsonError { at: start, why: "text that is not UTF-8".into() })?;
            out.push_str(run);
            match self.peek() {
                Some(b'"') => {
                    self.i += 1;
    ...
    fn number(&mut self) -> Result<Value, JsonError> {
        ...
        if self.s[int.start] == b'0' && int.len() > 1 {
            return Err(JsonError { at: int.start, why: "a number with a leading zero".into() });
        }
        let mut frac = 0..0;
        if self.peek() == Some(b'.') {
            self.i += 1;
        ...
        if matches!(self.peek(), Some(b'e' | b'E')) {
            ...
            let n: i64 = text.parse().ok().filter(|n: &i64| *n <= 10_000).ok_or_else(|| JsonError { at: start, why: "an exponent too large".into() })?;
...
mod tests {
    ...
    fn numbers_are_canonical_decimals_from_their_digits() {
        for (input, out) in [
            ...
            ("100", "100"),
            ("1050.00", "1050"),
            // a float's leftover digits, as a source writes them, kept to the last
            ("224.255004882812", "224.255004882812"),
