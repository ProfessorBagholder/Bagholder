...
fn a_run_stores_each_series_its_days_and_the_holidays_and_asks_again_only_when_due() {
    ...
    // the daily read covers to today; the noon read its own span
    let reads = book.rate_reads().unwrap();
    assert_eq!(reads[&cur("PLN")].iter().map(|r| (r.0, r.1)).collect::<Vec<_>>(), vec![(date(2026, 5, 1), date(2026, 9, 24))]);
    ...
    // after 16:30 Eastern on the 24th, the zloty is read forward; its rate is not out
    // yet, and the read still covers the day
    ...
    assert_eq!(book.rate_reads().unwrap()[&cur("PLN")].len(), 2);
