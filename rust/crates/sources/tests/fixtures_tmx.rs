...
fn row(ex: Date, record: Option<Date>, pay: Option<Date>, amount: &str) -> TmxDistribution {
    TmxDistribution { ex_date: ex, record_date: record, pay_date: pay, amount: dec(amount), currency: Currency::CAD }
}

#[test]
fn quu_lists_each_distribution_with_its_amount_dated_or_not() {
    let Outcome::Answered(rows) = tmx::parse_dividends(&common::json(TMX, "dividends-QUU.json"), "QUU") else { panic!() };
    assert_eq!(rows.len(), 39);
    assert_eq!(rows[0], row(date(2026, 9, 21), Some(date(2026, 9, 21)), Some(date(2026, 9, 28)), "0.91304"));
    assert_eq!(rows[38], row(date(2018, 3, 19), Some(date(2018, 3, 20)), Some(date(2018, 3, 27)), "0.07273"));
    let on = |d: Date| rows.iter().find(|r| r.ex_date == d).copied().unwrap();
    // the year-end notice of nothing
    assert_eq!(on(date(2025, 12, 31)), row(date(2025, 12, 31), Some(date(2025, 12, 31)), None, "0"));
    // rows listed with no dates: each amount as listed, whatever it was paid in
    assert_eq!(on(date(2023, 12, 28)), row(date(2023, 12, 28), None, None, "0.04676"));
    assert_eq!(on(date(2021, 12, 30)), row(date(2021, 12, 30), None, None, "0.43507"));
    assert_eq!(on(date(2021, 3, 22)), row(date(2021, 3, 22), None, None, "0.31301"));
    assert_eq!(on(date(2018, 12, 31)), row(date(2018, 12, 31), None, None, "1.11806"));
}
