...
fn yieldmax_states_a_weekly_schedule_and_rows_as_paid_at_the_time() {
    ...
    let err = payers::checked(Record { rows, per_year: Some(52), by_record: vec![] }, "2026-09-24T04:00:00Z".parse().unwrap()).unwrap_err();
...
fn defiance_lists_distributions_and_scheduled_dates_with_no_amount() {
    ...
    assert!(payers::checked(Record { rows, per_year: None, by_record: vec![] }, "2026-09-24T04:00:00Z".parse().unwrap()).is_ok());
}
