//! The loose-typed helpers the model leans on: activity rows arrive as
//! JSON maps whose fields may be absent, null, numeric or string, and every
//! reader has to agree on what that means.

use serde_json::Value;

pub const EPS: f64 = 1e-10;

/// Absent or null reads as the empty string, everything else as
/// its text. A number keeps the shortest form that round-trips, so an int is
/// written without a trailing `.0`.
...
/// Null, the empty string, a non-number and NaN all read as the
...
/// `compact`: upper-cased with whitespace, underscores and hyphens
...
/// and the non-breaking hyphen `_SPACE_RE` folds to one space.
pub fn is_space_like(c: char) -> bool {
    c.is_whitespace()
        || matches!(c, '\u{00a0}' | '\u{2000}'..='\u{200b}' | '\u{202f}' | '\u{205f}' | '\u{3000}' | '\u{2011}')
}

/// `norm_account_name`: runs of space-like characters folded to one
...
/// A float written with at least one decimal place, for the derived rows whose
