...
//! time under `ENV`'s lock. Shared by `tests/http.rs` and `tests/golden_fetch.rs`.
#![allow(dead_code)]

use bagholder_ws::session::Home;
...
impl Fixture {
    pub fn client(&self) -> bagholder_ws::session::Client<'_> {
        bagholder_ws::session::Client { home: &self.home }
    }
