...
impl TradeFig {
    ...
    pub fn gaps(&self) -> Gaps {
        let mut g = Gaps::none();
        for f in [&self.pnl, &self.pnl_cad] {
            if let Err(e) = f {
                g.merge(e);
            }
        }
...
fn collapse(inputs: &Inputs, matched: &Matched, key: TradeKey, trade: Option<TradeId>, trips: Vec<TripKey>, slices: Vec<Slice>, journal: JournalEntry, locked: bool) -> Option<TradeFig> {
    ...
    let qty = slices.iter().try_fold(Dec::ZERO, |a, s| a.checked_add(s.qty)).ok()?;
    ...
    let fees = slices.iter().try_fold(Money::zero(currency), |a, s| a.checked_add(s.entry_fee).and_then(|x| x.checked_add(s.exit_fee))).ok()?;
