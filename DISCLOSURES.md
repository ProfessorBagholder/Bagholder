...
- `internal/disclosures/disclosures.go` — the pipeline: the shared vocabulary
  (categories), the provider registry, and the merge. It dispatches by the
  instrument's market, calls each covering provider, and returns one sorted list plus
  a per-source status.
- `internal/disclosures/sedar.go` — the SEDAR+ provider. Reaches the site through
  `internal/browserhttp` (a browser TLS fingerprint, the only client its Radware bot
  gate admits); resolves an issuer to its profile, lists filings, downloads a PDF.
- `internal/disclosures/edgar.go` — the SEC provider. A documented JSON API, no gate,
  no key, the standard library only. Set `BAGHOLDER_SEC_UA` to your own contact (SEC
...
`bagholder mcp` exposes `disclosures_list`, `disclosures_document`, and
`sedar_resolve_profile` over stdio.

Claude Code:

    claude mcp add disclosures -- /full/path/to/bagholder mcp

Claude Desktop (Settings > Developer > Edit Config), under `mcpServers`:

    "disclosures": { "command": "/full/path/to/bagholder", "args": ["mcp"] }
...
`bagholder sedar` is a SEDAR-only utility for a shell or Claude Code:

    bagholder sedar resolve "Shopify"           SEDAR+ profiles matching an issuer
    bagholder sedar filings "Shopify" 50         an issuer's SEDAR+ filings (JSON)
    bagholder sedar newest 30                    newest SEDAR+ filings, any issuer
    bagholder sedar get <profileNo> <id> out.pdf download one SEDAR+ document

## What each filing is about

A filing list tells you the type, date and source, not what a document contains.
Two enrichments (`internal/enrich`) fill that in, both local. When a filing list is shown
...
- **Summary** — one plain sentence of what the filing announces, from a language
  ...
  `internal/enrich/localmodel.go` manages this. The Summary column itself appears only once a summary
  ...
  - The model needs the document's text. SEC filings are HTML and summarize out of
    ...
    decode, so their text comes from `internal/enrich/pdftext.go`: a system `pdftotext`
    (poppler) when present, otherwise the app's own reader, built into the binary, with
    nothing to install or run by hand. `BAGHOLDER_NO_PDF=1` turns extraction off.
