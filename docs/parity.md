...
- [x] The cards' pixel grammar against the original, line by line. *(The pixel comparison, 2026-09-25: both pages on the made-up book at 1200, 1340, 1440 and 1680 px, every tab, a trade, a holding, the ticket, the Orders and notifications panels, the menu and the filter; every text box measured for its position, size, font and colour, and the screenshots diffed. Fixed: the page drew in the system font, since it never loaded Inter (now shipped with the page, the variable font with its optical sizes, as the original was served); the space before `/` in Annualized returns; the trade and holding pages' 20 px margin; Disconnect red while greyed; the trade chart showing the library's default span instead of the trade framed. Differences left, each named: a time of day is the viewer's here and was the server's there (`SPEC.md`); on a holding whose id a closed trade also carries the server answers with the trade's executions, which goes with the switch, where the partly sold holding is the open trade; the chart library's logo sits in the pane instead of over the time axis; glyphs differ below a pixel between the font file served then and the one shipped now. Screenshot baselines held it through the switch and were then removed: what they guarded is held by the page's tests on figures and the DOM.)*
...
*Driven 2026-09-20 by `web/e2e/{dashboard,trades,portfolio,cashflow,marketcards,ticket,orderspanel,menu,filterfields}.spec.ts` (144 tests; 195 in all). What they found: the Annualized-returns benchmark was saved and never read back, so a reload lost it (fixed, `filters.svelte.ts`). What they could not reach, and why: a ticket opened from a listing outside the book and the external-listing search rows (need a directory lookup); the twelve-tile limit of the market tiles; the dimming of the other slices on a donut hover; News' Disclosures sub-tab. The chart is now reached: `demo-book --bars` stores daily bars for the listings traded, and the chart says in words what it drew (`role="img"`, bars and executions marked), which a test and a screen reader both read. The rest are owed canned quotes and directory answers in the test server.*
