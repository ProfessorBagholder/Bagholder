...
*Driven 2026-09-20 by `web/e2e/{dashboard,trades,portfolio,cashflow,marketcards,ticket,orderspanel,menu,filterfields}.spec.ts` (144 tests; 195 in all). What they found: the Annualized-returns benchmark was saved and never read back, so a reload lost it (fixed, `filters.svelte.ts`). What they could not reach, and why: fill markers and candles on the chart (a canvas, and the test server is offline so no bars arrive); a ticket opened from a listing outside the book and the external-listing search rows (need a directory lookup); the twelve-tile limit of the market tiles; the dimming of the other slices on a donut hover; News' Disclosures sub-tab. These are owed a test server that serves canned market data, which is the next thing the harness gains.*
