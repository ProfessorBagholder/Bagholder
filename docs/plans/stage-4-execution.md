...
1. **A trigger price far from the quotes around it.** The architecture says a bracket fires "never on a single tick far from the ones around it". Any "far" is a number nobody chose, which your decision of 2026-09-24 ("No hold on a quote that moved far from the last close") rules out for what is shown. For what the app *sends*, two ways: (a) a watched trigger fires only when two successive quotes, a few seconds apart, are both past its level; no threshold at all, and a real move costs one quote's delay (about 5 seconds) on the target and on a watched stop only; the stop Wealthsimple holds is untouched; or (b) no check, and the app fires on any one quote. **Recommendation: (a).** A bad print on Wealthsimple's quote would otherwise sell a position at market; five seconds is the cost, and the resting stop at Wealthsimple still protects the position meanwhile. If (b) is taken, the architecture's line is removed.
2. **Where the switch that stops all automated orders lives, and what shows while it is off.** The architecture asks for one switch that stops every order the app places by itself (bracket exits, trailing moves, renewals) without touching orders you place. Proposed: a switch in the Orders panel's header, `Automated` on/off, remembered; while off, each bracket leg the app would act on reads `Paused` where it reads `Watching` now, and nothing else is added. **Recommendation: take it as proposed.** Otherwise the only way to stop the engine is quitting the app, which also stops it guarding anything.

Everything else below is set by `SPEC.md`, the architecture and your decisions.

## Scope

Orders and brackets move from the old store into the book, each an explicit state machine whose state is what its event log says; every request to Wealthsimple leaves through one gate that records it, refuses it when orders are off, and never sends an order twice; a fill is booked at once as a provisional record that gives way to Wealthsimple's own row; nothing that follows live orders works from a list cut at a count; the guards the architecture lists (per-bracket order cap, stale-quote refusal, the switch above) are built; and the whole is tested against a fake Wealthsimple that loses, repeats, reorders and refuses. The page's Orders panel and ticket stop computing any amount themselves (the last allow-list entries in `web/src/no_money_arithmetic.test.ts`). What the person sees and does stays as `SPEC.md` §6 (Orders, Brackets) describes, except the two items above and the corrections listed under Approach.
...
**One gate** (`rust/crates/server/src/orders/gate.rs`, replacing `gql_as` for order mutations): takes an order request and who asked; refuses when orders are off (dry), when the automated switch is off and the asker is the engine, when an order for the same bracket leg is in flight, or when the bracket has sent its cap this minute; appends the request event before sending; sends once, never retried by the transport; appends the answer as received, or `sent, not confirmed` for anything else. The ticket, the Orders panel's routes and the bracket engine all go through it; `bagholder_ws::session::Client::graphql` is no longer reachable for a mutation from anywhere else (a boundary test, as 3c's).

**Guards:** per bracket, at most 10 orders a minute; one more trips it: the bracket is stopped (`stop watched`, nothing sent), and the person is told in the header, because that many can only be a fault (a trail step is cancel + new, two orders, and moves at most every five seconds only in a runaway). A trigger fires only on a Wealthsimple quote whose own time (`quotedAsOf`) is within 15 seconds; an older one is not acted on, and the leg reads `Paused` meanwhile. Item 1 above for a tick far from the others.
...
- §6 Failures: "There is no other throttle" → the per-bracket cap, and the automated switch (items 1–2).
- §6 Orders panel: the switch (item 2).
...
- [ ] **Guards:** the 11th order a bracket sends within a minute is refused, the bracket goes to `stop watched`, and the header says so; a quote older than 15 seconds by its own time fires nothing; with the automated switch off, the engine sends nothing and the person's own orders go through; and item 1's rule, as decided.
...
- [ ] **Rendered:** the ticket and Orders panel on the Rust scratch server (`SPEC.md` §7) on a copy of the owner's book with orders off: every amount traced to its server field, no overflow at 1200 / 1340 / 1440 / 1680; the switch and `Paused` as decided.
