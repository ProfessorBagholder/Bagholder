...
- **What the market's record does not state is not read into it.** TMX lists some distributions with no pay date and does not say whether any is paid in cash or in units; the reader built for one fund family read a row with no pay date as paid in units, which held for that family's year-end distributions and not for a cash payment TMX lists without its dates. As the record for any payer, a row that does not state its form is stored as unstated (book migration 007, a `form` of `cash`, `units` or `unstated`), and the rate waits on it (`form-unstated`) until the account's own payment for that ex-date shows cash or units, which the engine reads from the record.
- **Test:** on recorded replies, a payer no company reader serves gets its record and schedule from the market's (a Canadian listing and a US one); a company reader's failed read keeps its record, is named, and asks the market nothing; one whose publication does not carry it has the market's record; a row of unstated form makes the rate wait until the account's payment for it shows its form.
