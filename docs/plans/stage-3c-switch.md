...
- **Parity locked first** (brief 01 §2.4): the pixel comparison with `ledger.html` at 1200, 1340, 1440 and 1680 px on the demo book, its differences fixed, then screenshot baselines of every tab, the trade and holding pages and each overlay at the four widths, taken on the old path. From then on a baseline changes only where `SPEC.md` changes, named in the commit. *Departure (2026-09-25):* the baselines were removed once the switch was done. Every intended change meant regenerating and reviewing pictures, and what they guarded, figures and screens changing only where `SPEC.md` says, is checked directly by the page's tests on figures and the DOM.
...
- `web/src/lib/generated/*.ts` (wire, routes, book, model_api) regenerated; `web/e2e` baselines (removed after the switch).
...
- `.github/workflows`: the baselines' job (removed after the switch).
...
**Step 1, parity baselines (9eb0cf31, 307f9aed).** Both pages on the made-up book at 1200, 1340, 1440 and 1680 px: every tab, a closed trade, a holding, the ticket, the Orders and notifications panels, the menu and the filter; every text box compared for position, size, font and colour, and the screenshots diffed. Fixed: Inter was never loaded by the Svelte page (now shipped with it: the variable font with its optical sizes, as the original was served); the space before `/` in Annualized returns; the trade and holding pages' 20 px margin; Disconnect red while greyed; the trade chart kept the library's default span instead of the trade framed. After the fixes every tab matches box for box at every width; the differences left are named in `docs/parity.md`. The 48 baselines were made in the CI job `baselines` (the Playwright image, the server's clock fixed by faketime) and compared there on every run until the switch was done. The last review (run 36196296996) traced every difference to a named change: realized parts and the Equity switch on the Dashboard; quotes, ex-dates, the YTD sort and four places under a dollar on Cashflow; open trades and the options venue on Trades; the made-up buying power on Portfolio. It found three defects, fixed in 34026623, 2941ed5c and cef21120: the made-up coins had no bars, a holding's chart waited on the sources and was framed on its entry, and same-day distributions swapped order. The job and the baselines were then removed (§1).
...
**Step 7, the Equity switch.** The engine's scoped figures carry the realized P&L in scope as a running total by the day each part was realized (`PnlCurve`), a part that waits left out and counted, a total that does not fit a gap from that day on; the value series names the filters set that it does not read (`Filters::unread_by_value`: all but the account). The card opens on `P&L`, remembers the choice in the browser, reaches below zero where the P&L does, and under `Value` names the filters it does not read. Tests: the curve's days, left-out parts and overflow, and the filters named (`scope::tests`); the P&L curve ends at the Realized tile's figure under no filter, a symbol, an account, a grade and a year, the card's switch, its axis below zero, its note and the choice surviving a reload (`e2e/dashboard.spec.ts`). The Dashboard baselines change here: the card's title row carries the switch and opens on `P&L`.

**Step 8, the old path removed.** *Departure from §8, argued:* the earlier model and the earlier store's readers of its figure tables are what read an earlier database: the first start's import carries its journal by the earlier engine's trade keys (`legacy_import.rs`), and `compare-figures`, which the real run ends on, measures the engine against the earlier model on the person's own earlier file. They stay, as readers of an earlier database file only, and the boundary test keeps them off the figure path; what goes is everything the running app did with them. Done so far: (1) Margin Boost read by the adapter (1f3144d6). (2) The running app no longer builds the earlier model: the market's context (`bagholder_model::context::MarketBase`) is the engine's holdings, valued in CAD, and round trips, with the context tables of the earlier store, rebuilt only when one of them moved (`server/src/market_context.rs`); the routes the page no longer calls are gone (`/api/model`, `/api/trade`, `/api/book`, `/api/book/append`, `/api/groups`, `/api/notes`), and with them the model cache; the earlier refreshers of the rates, benchmarks and distributions, which the scheduler reads (`due.rs`), no longer run, and the quote loop reads only what the context shows (watched listings and tiles), a holding's price being the scheduler's. (3) What still read the earlier store's figure tables at run time reads the book: a watched bracket's sale or vanished position (the book's records and statements of units, `Book::units_reads`), an order's name and venue, a distribution release's notice (the book's declared record, read again through the scheduler's reader), the exposure and filings sweeps' securities. (4) The earlier store's figure tables (`bagholder_store::schema::FIGURE_TABLES`) are dropped from the live file at start once the book holds their rows (refused while it holds activity the book never imported), the file snapshotted first to `snapshots/bagholder-before-the-book-<ms>.db`, which `compare-figures` reads (it refuses the live file, naming the snapshot); the schema and its repairs make none of them again. Tested on a copy of an earlier database (`legacy_import::tests`). (5) The earlier Wealthsimple sync, its mapping and its reads are gone from `bagholder-ws`, which keeps the session and what orders send until stage 4; the element diff is its own crate (`bagholder-diff`), the earlier model re-exporting it; the machine-zone midnight signal is gone, the day turning in the person's zone being the scheduler's. (6) The boundary test (`server/src/tests_boundary.rs`): no file on the figure path (the wire but its context door, the figures and the engine's inputs, the scheduler and the sources' needs, the broker's reads, entries and imports, the stream and the figures' routes) names `bagholder_model`, `bagholder_store`, `bagholder_ws` or `bagholder_market` outside its tests; checked by feeding it a violation.
...
- **Margin boost**: settled in step 8. The accounts' read asks for the feature's metadata, reads it strictly (a boost that is on names a margin account the reply holds, or the read fails), and the book keeps which account backs which (migration 010, `margin_backing`, replaced by each read); the ticket reads it from the book.
