...
- [x] With no history source, the priced executions are not plotted on a time axis. Old page same. — fixed: where the server answers that no source covers the instrument, the priced executions stand at their prices on a zoomable time axis, marked as on the bar chart, nothing joining them.
