...
- `cargo test --workspace`: 765 passed, 0 failed, 1 ignored; `cargo build --workspace --all-targets`: no warnings. The ignored test, the Mac notifier applet, builds a real applet with `osacompile`, which needs Apple Events; the sandbox this stage was built in refuses them, so it could not run here. Stage 2 does not touch `crates/server/src/notify.rs` (no diff since stage 1's `828a164`); run it outside the sandbox with `cargo test -p bagholder-server notify::tests::test_the_mac_applet_is_built_once_under_bagholders_name_and_icon -- --ignored`.
