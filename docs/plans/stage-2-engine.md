...
A figure the engine cannot state is a `Gap` naming the fact it waits for, never zero and never a guess: `rate-pending`, `rate-missing`, `rate-unpublished`, `multiplier-unstated`, `quantity-unstated`, `leg-unstated`, `event-unknown`, `basis-unknown`, `beyond-held`, `effect-conflict`, `currency-unstated`, `value-unstated` (a fill stating neither cash nor price), `adjustment-conflict` (two sources explaining one transaction differently, neither superseding), `price-unknown`, `close-unknown`, `frequency-unknown`, `distribution-unknown`, `no-expiry-record`, `record-problem`, and `arithmetic` (a figure too large to hold exactly, or a division with nothing to divide by, named in words). The page (from stage 3) shows `—` and the one word in the figure's place, as it does now for a deposited coin. A tile over many trades or positions is the sum of those whose figure is stated, and its subtitle adds how many it left out (`2 trades waiting`); the count is the engine's.
...
Made with the switch in stage 3, since until then the app the person runs still does what `SPEC.md` says: trade identity (Bagholder's trade ids, lots by instrument; `docs/architecture.md` §18); options matched as a net position, rolls only as one order's legs (the old same-day folding of separate orders goes; the person groups trades for that); splits and multipliers from the record and the contract; expiry without a record; assignment's delivery; transfers between accounts; the rate rule for transactions and for live marks; the position's price without the person's fills; payout frequency never assumed; §2's "Distribution rate for a holding" rewritten (the payer's own record, not TMX; the cash per unit of the latest distribution gone ex; the schedule the payer states, never worked out from ex-date gaps and never 12 assumed; the exchange-side record only for a fund company whose own publication cannot be read, stage 3a); the equity series and the broker check; tiles as the sum of what is stated with the count left out (a change to §1's "either exactly what this file defines or not shown", which now reads that a tile states what it covers); per-month figures from the engine.
...
- [x] Rust, from `rust/`: the whole workspace's tests green and a warning-free build; then the ignored Mac notifier test alone.
- [x] The boundary test fails when the engine gains a dependency other than `bagholder-core`, reads the clock, or uses `f64` outside `engine/src/stat/`, each checked by feeding the checker a violation.
- [x] Every case in `crates/engine/tests/cases` passes; the cases cover every item under *Cases written from the spec*, each with its working.
- [x] The invariants hold on every case they cover: the parts of a lot or fill add up to it exactly (all); a closed round trip's P&L equals the sum of its fills' cash (round trips whose fills all state cash and whose lots were never moved at cost or delivered without stated cash); a position's quantity equals the sum of its account's transactions in that instrument (holdings with nothing waiting and no split marker applied by ratio). A record whose stated price and cash disagree is listed as a problem, a case each way.
- [x] The order within a day, each a case: an undated purchase before the morning's timed sale, an undated sale after the day's timed purchase, a short opened and covered at one instant, a roll's legs at one instant, and the same book stored in two orders giving the same figures.
- [x] Rates, each a case: a weekday a completed read covered without a rate takes the previous day's; a weekday no read covered is pending before 16:30 Eastern and a failure after; a live mark uses the latest published rate and is a failure once a later business day's 16:30 has passed without its rate.
- [x] Every variant of `Change`, applied to a built engine, equals a fresh build field for field, and `Moved` names exactly the entities and fields that differ (one test, a `match` over the enum).
- [x] No old guess remains in the engine: no quantity inferred from cash, split inferred from prices, ticker-replacement search, fixed multiplier, fee tolerance, residue rule, fallback rate or position marked at a fill; a test reads the engine's sources for each, and a case holds what replaced it.
...
- [x] Migration 2: a book at version 1 migrates to 2 with its rows intact and every trade's anchor instrument filled from its anchor; `schema/v2.sql` is committed and compared; each new table's typed write and read is a test; a second different rate for a stored day is kept, reported, and does not replace the first; a read's span is kept; a withdrawn distribution is absent from the newest read; a sourced adjustment supersedes the person's; a supersede moves an adjustment's `applies_to` with the record, as it moves a trade's anchor.
- [x] Import version 2: each relabelled shape (an option buy and sell under an order label, a multi-leg debit and credit, an assignment) maps as above, a fixture test each; re-deriving a version-1 book changes exactly those transactions.
- [x] The comparison on copies of both of the person's databases: every difference attributed; for each cause, the sample checked against the records written in Verification; the counts in Verification.
- [x] `docs/design-review.md` records stage 2 as done, and records as stage 3's: the switch without a bridge, the fact readers, the `SPEC.md` changes listed under *Changes to `SPEC.md` at the switch*, where the live-mark rate's date shows, the wire's number format, what Clear data does to the book and its journal, the home zone taken from the page, and how far back the Bank's series go against the person's oldest day.
...
Run on 2026-09-23 in the worktree, from `rust/`.

- `cargo test --workspace`: 765 passed, 0 failed, 1 ignored; `cargo build --workspace --all-targets`: no warnings. The ignored test, the Mac notifier applet, run alone with `--ignored`: passed.
- The boundary test's checker is fed a dependency other than `bagholder-core`, a clock read and an `f64` outside `engine/src/stat/`, and reports each (`crates/core/tests/boundaries.rs`).
- 20 case files in `crates/engine/tests/cases`, every case with its working, all passing. Beyond each case's own figures, the runner checks on every case: no two positions share a key; a closed round trip whose fills net to no units, all in its own currency, and whose instruments no event moved, has its fills' cash as its P&L; a holding moved only by its own trades, rewards and unlinked transfers holds what its transactions add up to; and the same book listed in reverse order gives the same figures (`Engine::differences` empty). These found one bug, fixed: an opening against an opposite position joined that position's round trip.
- `tests/changes.rs`: every `Change` applied to a built engine equals a fresh build, and `Moved` names exactly what differs. `tests/no_guesses.rs` reads the engine's sources for each old guess.
- An independent code review (Opus) raised 20 findings; each is fixed with a case or a test that fails on the code before the fix, run to confirm.
- The comparison, `bagholder compare-figures` on copies of the person's databases (`rs.db` and `py.db`) against the old model, as of 2026-09-23. Re-deriving with import version 2 changed 135 transactions (71 buys, 61 sells, 3 dividends), nothing added or removed. Trades: 229 old, 289 new, 162 the same to the cent. Realized P&L by instrument: 126 agree to the cent, 33 differ. Positions: 12 old, 42 new, 11 the same. All-time dividends: 87,688.38 old, 87,688.60 new. Every difference is attributed, and a sample of each cause was checked against the records:
  - multi-leg orders the import keeps without their legs: the contracts on their underlying wait (`leg-unstated`);
  - contract sizes not yet stated, including the deliveries of LUNR, ASTS, SMCI and QBTS assignments (`multiplier-unstated`), which stage 3's contract reader states;
  - holdings renamed or consolidated under a new security id with no event row (CH, QIMC→QESS, MSTY 1:5): the old model searched by ticker; here the sale is beyond what the new instrument held until stage 3 states the event;
  - coin dust sold beyond what was held, and coins deposited with no cost on the record (`beyond-held`, `basis-unknown`);
  - the old model folded separate same-day orders into one and relabelled every option fill "to open" (hand-checked: LUNR 01AUG25 12C, −1,944);
  - round trips now end when the position is flat in real time, so the dashboard counts 255 trades where the old model counted 215;
  - a few cents of dividends in CAD from the rate on the filing day;
  - the KXFED event contract (−67.67) now counts;
  - prices come only from quotes and closes, never from the person's fills;
  - one PEPE round trip (−17.63 in the old model) now waits (`value-unstated`): a dust sale of 0.413116 PEPE whose imported row carries a zero, which the import reads as not stated because the old app stored missing cash as zero. Wealthsimple's own row in stage 3 states it.

## Handoff

Stage 2 is built on `svelte-migration`; nothing changes on screen, because the app still runs the old model until the switch in stage 3. The engine, the book's migration 2 and import version 2 are in place, with the comparison tool to run them beside the old model on the person's data. Stage 3 starts from `docs/design-review.md`: the broker interface with Wealthsimple's raw rows (stating multi-leg legs, account links, contract sizes, events, and when each account's activity was last read, which the broker check measures a statement against), the strict fact readers, and then the switch with the `SPEC.md` changes listed above. The comparison's causes above are the list of figures the person will see change at the switch.
