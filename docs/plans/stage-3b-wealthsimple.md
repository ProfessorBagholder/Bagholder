...
- Suite, from `rust/`: `cargo test -q --workspace` 1,106 passed, 0 failed; `RUSTFLAGS="-D warnings" cargo build -q --workspace --all-targets` clean.
...
- The comparison run again (`compare-figures … --facts-from-book`), rows now joined by Wealthsimple's own id for them (the old store gives its rows new ids of its own each time it syncs, so its ids named nothing across copies and every pairing read 0): trades 161 of 229 the same to the cent; payments 255 of 270, the 15 others differing in CAD by cents (the day's rate); positions 12 of 13; realized P&L by instrument 106 agree, 62 differ; all-time dividends 89,723.38 old, 89,723.60 new; portfolio market value 627,165.03 CAD with 3 left out (3a's run: 632,070.43 with 15). The differences, by cause:
  ...
  - 133 new round trips in the managed accounts, which the old model left out; those buying a USD security wait on its cost in USD, since a managed row states only the CAD paid (`currency-unstated`). Whether a managed account's trades are the person's trades is the open question below.
...
Stopped at: the rules, the check and the comparison done. Left: the real run on a live sign-in, the one step left to the owner; the question below.

Open for review: whether the trades Wealthsimple makes in a managed account are the person's trades. The new model makes round trips of them (133 on the owner's history); the old left them out. A managed row states the CAD paid, not the price or rate in the security's currency, so a USD holding's cost waits (`currency-unstated`). The journals compared (TradeZella, Tradervue, TraderSync) journal the trader's own executions; a robo-advisor's rebalancing is not one. Proposed: a managed account's rows stay on the record and count in cash, holdings, value and the broker check, and make no round trips.
