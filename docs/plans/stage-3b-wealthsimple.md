...
- Suite, from `rust/`: `cargo test -q --workspace` 1,031 passed, 0 failed; `RUSTFLAGS="-D warnings" cargo build -q --workspace --all-targets` clean.
...
- The broker check on the real run: 16 of 29 accounts agree exactly. Every difference traced to its cause:
  ...
  - Moves between accounts whose holdings were sold into cash before the move (`moved-holdings-unstated`, 8 rows): two "full in kind" moves that state no amount (±9,307.07 between two closed accounts; the group RRSP's 6,310.66 into the RRSP) and two "partial in kind" moves out of the crypto account (5,925.41). Their positions show no holding arriving: the moves are cash. The capture lacked what settles them (the no-amount moves' detail, and the positions of the day before for the others); a live pull reads both.
  ...
  - Fixed: the card's 35 purchases of May 2026 counted twice. Wealthsimple lists a card purchase under one id while pending and under another once posted; the import held the pending ones, and the full read, which lists only the posted, did not replace them. A pull now removes what the broker no longer lists over the span it read (`wealthsimple/tests/pull.rs`).
  - Found: a transfer in from another institution states the amount asked for, not the amount that arrived. The locked-in account's row states 80,807.80; its net deposits rose by 80,650.30, and the account closed at zero only with the second (157.50 less: the other institution's transfer-out fee of 150.00 plus GST). The transfer's detail (`FetchInstitutionalTransfer`) states the amount that arrived per currency once completed (`actualValueLegCad`, `actualValueLegUsd`), and whether it moved cash, holdings or both (`transferType`); the web app shows the actual value when the transfer is completed and the expected one before. To be read with the row, like an internal transfer's detail, once its real reply is captured.
  ...
  - Not compared: the credit card. The balances reply lists no balance for it; its balance is `creditCardAccount.balance` (`FetchCreditCardAccount`), to be read for the check once its real reply is captured.
...
Stopped at: the replies the capture lacks, asked of the owner in one console paste (`console-gaps.js` in the session scratchpad): the institutional transfers' detail and selected assets, the card's balance, the two no-amount moves' detail, four days of positions, a refusal and a lapsed session. What is left, in order:
1. With the replies: read an institutional transfer's detail with its row and book the amount that arrived per currency (in kind: its holdings, read against positions as a move between accounts is); read the card's balance into the check; the refusal and lapsed-session replies into `wealthsimple/tests/session.rs`; re-run the real pull and the check.
2. A statement's units in an instrument no row names: make the instrument from the broker's security record (read in the securities batch) so the check shows it as a difference beside the instrument it replaced, rather than a failure that leaves it out.
3. `compare-figures` now names a broker record by the imported row it replaced (`record_refs`); per-trade pairing still reads 0 the same, which points at the round trips' boundaries rather than the keys (the old model split a position's trips where the new one keeps one open to flat): pair by instrument and close instead, then read the differences.

Not to redo: the mapping's rules, the capture, the anonymiser, the pull and its request counts.

To resume: `cd rust && cargo run -q -p bagholder-server --bin bagholder -- pull-broker $TMPDIR/bh-3b/run4 --replay <capture as sent>` and `compare-figures $TMPDIR/old-ro.db $TMPDIR/bh-3b/run4 2026-09-24 --facts-from-book --cache $TMPDIR/bh-3b/run4/market.db`.
