...
1. **Recording Wealthsimple's real replies needs one sign-in, to a scratch copy of the new build.** The adapter is tested against what Wealthsimple really sends (`docs/architecture.md` §9), so its replies have to be captured once from a signed-in session. Recommendation: you sign in once to the new build running on its own scratch data folder, the one step only you can do. Reusing the running app's saved login instead would rotate its refresh token (Wealthsimple accepts each refresh token once), and your app would be signed out. If this is not taken, the adapter can only be tested against invented replies, which is what the current code does and what the design rejects; the plan stops after research.
...
Part of each is already answered from Wealthsimple's own web app: its public bundle (release 0.3.668812, read 2026-09-24 without signing in; 991 operations and 669 fragments recovered). What remains needs the person's real replies (decision 1), for rows chosen for each question: a multi-leg roll (the BBAI rolls), an assignment (LUNR, ASTS, SMCI, QBTS), the BBAI put of 2025-12-26 finishing in the money, the MSTY 1:5 consolidation, the CH and QIMC→QESS continuations, the PLUG corporate action, an asset movement between accounts, an institutional transfer in, a crypto transfer in, and a distribution that may carry return of capital. Each answer, and the row that gave it, goes into Verification before its reader is built. Why this way: the bundle shows what can be asked without touching the account; the person's own rows are the events the figures wait on (`docs/plans/stage-3a-sources.md`, Verification), so each is settled by the row it is about. The old store cannot answer any of them: its corporate-action rows for MSTY and PLUG read quantity 0 and its multi-leg rows carry the old app's relabelled sub-types.

1. **Multi-leg orders' legs.** Objective: each leg read as stated, never from the cash's sign. **Found in the bundle:** the activity row carries no legs; for an `OPTIONS_MULTILEG` row the web app passes its `externalCanonicalId` as `orderBatchId` to `FetchSoOrdersMultilegOrder` (`branchId: "TR"`), which states `optionStrategy` and each leg's `securityId`, `side`, `openClose`, `filledQuantity`, `averageFillPrice`, `filledNetValue` and fill times. **Remains:** that a real multi-leg row's batch id answers, and that the legs' fills add up to the row's cash.
2. **Contract sizes and the OCC symbol.** Objective: `option_terms.multiplier` as stated, and an adjusted contract found by its OCC symbol (`docs/plans/stage-3a-sources.md`, Handoff). **Found:** the security's `optionDetails` states `multiplier` and `osiSymbol` (with `underlyingSecurity`, `expiryDate`, `strikePrice`, `optionType`); no `deliverable` or "adjusted" field exists on a security. **Remains:** a real reply for a held contract and for an assigned one.
3. **Corporate events.** Objective: an event's units from the broker's own statement of what moved. **Found:** the row carries dates and a dividend rate only; the web app reads each event's detail from `FetchCorporateActionChildActivities(activityCanonicalId)`, which states per entitlement (`SUBMIT`, `RECEIVE`, `HOLD`) the asset, its type (`EQUITY`, `OPTION`, `CASH`), quantity, currency and price. No ratio, old or new security id, CUSIP or ISIN is stated anywhere. **Remains:** the MSTY, CH, QIMC→QESS and PLUG events' real replies: whether the units given up and received, and any cash in lieu, are each stated, and how the received asset is tied to a security id.
4. **Positions and balances now.** Objective: the broker check compares the book with the broker's own statement of cash and units (`engine/src/equity.rs:99`). **Found:** `FetchIdentityPositions` (`PositionV2`: quantity, direction, `bookValue`, `averagePrice`, security, legs) states no time, so a statement's `as_of` is the instant it was asked for; `FetchHoldingsExportPositionsAsOfDate` states positions (quantity, direction, book value, close) **as of a given day**. Taken: units are checked against the positions as of the last day whose activity is read in full, which removes the "not yet posted" case from the unit check; cash stays checked against the balances now (`FetchAccountsWithBalance`) with `as_of` and `activity_read_at`. **Remains:** real replies for both.
5. **When an account's activity has been read in full.** Objective: `activity_read_at` means every row Wealthsimple had posted by then is on the record. **Found:** the activity condition the web app sends uses `accountIds`, `startDate`, `endDate`, `types`, `subTypes` and `unifiedStatuses`; nothing filters by when a row last changed, although rows carry `unifiedStatus`. Taken: each pull reads each account's whole feed, which is exact with nothing assumed; its cost (requests per pull on the person's book) is counted in Verification. An overlap in days is not used: no measurement could bound how late a row may be posted or revised.
6. **Transfers between the person's own accounts.** Objective: a holding moved between accounts keeps its round trip, linked by the broker's ids, never by amounts. **Found:** the row states `opposingAccountId` and `transferType`; `groupId` is on every row. **Remains:** on a real asset movement's two rows, which id joins them and whether each states units and security.
7. **The cost of a transfer in.** Objective: a holding transferred in carries its cost when Wealthsimple states it, and only otherwise waits for the person's entry. **Found:** no activity row states a book value; a position states `bookValue` (Wealthsimple's own book cost for the whole position, which carries over on an ATON transfer per its help centre, `briefs/reference.md`); an incoming institutional transfer's tasks state `bookValue` only for assets the person must decide on. **Remains:** whether a real transferred-in position's stated book value can be tied to the transfer's units (it can only where the position has not changed since, which is checked, never assumed); where it cannot, the person's entry.
8. **The daily history's operation.** **Found:** the identity-wide `IdentityHistoricalFinancialsQuery` the current code asks no longer exists in the web app (now `FetchIdentityHistoricalFinancials`); `FetchAccountHistoricalFinancials` states `historicalDaily { date netLiquidationValueV2 netDepositsV2 }` per account, which is all the engine reads. Taken: only the per-account operation is asked.
...
- **Multi-leg orders**: each leg as the batch states it (question 1): its contract, side, opening or closing, filled quantity and price. A leg whose quantity or side is not stated keeps `leg-unstated`.
...
- [ ] **Research.** Questions 1–8 answered in Verification with the replies that answered them (kept as fixtures) and the decision taken; no reader built on a question left open.
