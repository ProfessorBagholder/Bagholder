...
Stopped at: the rules, the check and the comparison done. Left: the recorded replies' wrong-shaped and wrong-meaning copies for every operation (two operations have them); the real run on a live sign-in, the one step left to the owner; the question below.
