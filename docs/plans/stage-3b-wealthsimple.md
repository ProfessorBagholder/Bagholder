...
4. **Positions.** Objective: the broker check against the broker's own statement. Settled: positions now carry no time (their `as_of` is when they were asked); positions as of a given day (`FetchHoldingsExportPositionsAsOfDate`) state quantity, direction, book value and close per security. Units are checked on the last day whose activity is read in full; cash against the balances now, a credit card's against its own balance (`creditCardAccount.balance.current`, what is owed). A holding stated in an instrument no row names (a token migrated to a new id, a gift) takes its instrument from the security's own record, read with the others of its statement in one batch, so the check shows it beside the instrument it replaced; a statement is stored only whole.
...
3. **Activity.** As question 5 sets (in full once, then only what changed), each row stored as a source record (source `wealthsimple`, key its `canonicalId`, the payload canonical JSON), a changed row as a revision (`book/src/records.rs:173`). An imported record carrying the same id (`broker-record:wealthsimple`, `book/src/import/mapping.rs:35`) is superseded by it (`records.rs:118`), and its trade anchors move with it (`book/src/links.rs`). A row the broker no longer lists over a span it read in full is removed, and listed in the output (a pending card purchase is posted again under another id). A final row gone, older than the rows read again or more than three in one account, removes nothing for that account and reports the read suspect (brief 08). A row the adapter cannot read is kept as a record with the reason and read again; a row with no id of its own leaves its account's read incomplete, named.
...
- **The day a row is filed under** is Toronto's (`docs/architecture.md` §7), as the import mapping applies too. *Corrected 2026-09-25:* this said Alberta's, never checked; Wealthsimple stamps a date-only row at midnight Toronto, so Alberta's day put each month's margin interest, charged at midnight on the 1st, into the month before.
...
- [x] **Recorded replies.** Every operation the adapter asks has recorded real replies (an answer, an empty answer, a refusal, a lapsed session where Wealthsimple gives one), each with a wrong-shaped and a wrong-meaning copy, and a test per reply asserting exactly what is written, or that nothing is and which outcome is recorded. The anonymisation test passes on every one.
...
- [x] **Records.** On recorded replies: each row stored once as a source record, a changed row as a revision, an unchanged row as nothing new; an imported record with the same Wealthsimple id superseded and its trade and journal kept (the trade id unchanged); a row the broker no longer lists over a span it read removed (Wealthsimple posts a pending card purchase again under another id; changed from "not marked removed" when the real run found the pending ones counted twice).
...
- [x] **The comparison run again**, its numbers in Verification against 3a's run, and each of 3a's waiting gaps resolved or named with what Wealthsimple states.
- [x] **Docs.** `SPEC.md`, `docs/architecture.md` (if the interface departs from §10), `docs/old-app-mistakes.md` (the guessed overlap), `docs/design-review.md` (the order of work) and the cases README say what the code does.
...
- Suite, from `rust/`: `cargo test -q --workspace` 1,114 passed, 0 failed; `RUSTFLAGS="-D warnings" cargo build -q --workspace --all-targets` clean.
...
- The broker check on the real run: 21 of 29 accounts agree exactly. Fixed on the way, each held by a test on the owner's own replies:
  ...
  - Two coins the crypto account holds as dust after token migrations (RNDR to RENDER, FTM to S) that no row records. A pull now makes their instruments from their securities' records (`tests/pull.rs`); the capture holds no record of these two, so on the real run the crypto account's units statement names them and is not stored, and its units are not compared until a live pull reads them.
...
- Recorded replies (`wealthsimple/tests/replies.rs`, `tests/replies/wealthsimple-ops/`): every operation's answer, empty answer, refusal and lapsed session, and a wrong-shaped and a wrong-meaning copy of each answer built from the real one by one field, a test each (70). Readers made strict on the way: a position whose quantity's sign contradicts its direction, a contract on zero units, an executed multi-leg row whose order states no leg filled (`legs-disagree`), a corporate action whose entitlements state nothing (`event-unstated`), a funding intent's received side against its row (`conversion-disagrees`), a card reply for another account, an account the balances answer leaves out (its cash left unstated, never zero), and a row status the web app does not list (every one it lists is placed as final or read again). The whole history maps as before under them.
...
- The comparison run again (`compare-figures … --facts-from-book`), rows now joined by Wealthsimple's own id for them (the old store gives its rows new ids of its own each time it syncs, so its ids named nothing across copies and every pairing read 0): trades 161 of 229 the same to the cent (311 new, the managed accounts' round trips gone); payments 255 of 270, the 15 others differing in CAD by cents (the day's rate); positions 12 of 13; realized P&L by instrument 106 agree, 62 differ; all-time dividends 89,723.38 old, 89,723.60 new; portfolio market value 627,165.03 CAD with 3 left out (3a's run: 632,070.43 with 15). The differences, by cause:
  - Where a round trip starts and ends: the new model keeps a position's trip open to flat (32 trades matched a different quantity; the realized total by instrument is the check apart from the bounds).
  - Options rolled: the new model reads a multi-leg order's legs and their cash; the old booked every roll's cash backwards (`docs/old-app-mistakes.md`).
  - Coins deposited without a cost wait on the person's cost of arrival (`basis-unknown`), where the old model made one up; coins whose fills state fewer digits than their positions show a sale beyond what is held by a fraction of a unit (`beyond-held`).
  - A holding under a new security id with no event row (above).
  - 133 new round trips in the managed accounts, which the old model left out; those buying a USD security wait on its cost in USD, since a managed row states only the CAD paid (`currency-unstated`). The owner decided (brief 08): a managed account's trades make no round trips, so these leave the comparison.
...
Stopped at: accepted (brief 08) with three fixes, made: one unreadable row no longer stops its account (`tests/pull.rs`); removals listed and guarded (`tests/pull.rs`); a move with no amount takes a stated amount first, and otherwise its net deposits only within the week after it, from exactly one day of opposite changes once the days other moves between the two accounts state are set aside (the owner's history has one: a 1.58 move two days after the 9,307.07) (`tests/mapping.rs`). The owner's decision on managed accounts is in `docs/decisions.md` and `SPEC.md` §2 (`engine/tests/cases/open_trades.json`). Left: the real run on a live sign-in, the owner's one step, with 3c.

Not to redo: the mapping's rules, the capture, the anonymiser, the pull and its request counts.

To resume: `cd rust && cargo run -q -p bagholder-server --bin bagholder -- pull-broker $TMPDIR/bh-3b/run8 --replay <capture as sent>` and `compare-figures $TMPDIR/old-ro.db $TMPDIR/bh-3b/run8 2026-09-24 --facts-from-book --cache $TMPDIR/bh-3b/run8/market.db`.
