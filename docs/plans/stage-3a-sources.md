...
- **A company's own announcement** states each dividend it declares: the amount, the record date, the pay date, and the schedule in its own words ("declared a quarterly dividend of …"). One adapter per publication form (Cision's newswire.ca pages, the InvestorRoom/MediaRoom sites, Q4's feed), each company's sentence read strictly. The ex-date is the exchange's rule from the record date: the record date itself from record date 2024-05-28 (TMX's T+1 notice gives record date 2024-05-27 the ex-date 2024-05-24), one session before it from record date 2017-09-07 (TSX notice 2017-018), two before that, counted on the exchange's sessions: the days XIC, the Composite's tracker, has a close, read from Yahoo for the fortnight before the record date where the cache does not hold them. Until those sessions are held, nothing of the company is stored and it stays due.
- **What a publication states, kept as stated.** A distribution's cash and reinvested parts where the payer states them (BMO, Vanguard, iShares Canada, Mackenzie, Global X's reinvested table, Fidelity's release); a row repeated identically is one row; two different rows for one ex-date, or a date outside the fund's life, are a meaning failure naming both, and the read writes nothing. A record date more than a week from its row's stated ex-date is a slip in the page (seen: a year typed ten years out on one row of a fund's table), not the distribution: that date alone is not taken and nothing stands in for it, since no figure uses a record date where the ex-date is stated; one cell never costs the payer's record.
...
- [x] **Payers' publications**, for each fund company and announcement form, each a test on its recorded pages:
  ...
  - a company release's amount, record date, pay date and schedule read from its own sentence, its ex-date the record date from record date 2024-05-28 and one or two of XIC's sessions before it earlier (2023-10-10's is 2023-10-06, Thanksgiving between), and nothing stored while those sessions cannot be read;
...
- every recorded reply is asserted by a test, and every source has a wrong-shape and a wrong-meaning copy where its reader has a check an edit can trigger (Defiance states no schedule, so it has no schedule word to get wrong);
...
- **Option closes.** Superseded by the owner's decision of 2026-09-24 (`docs/plans/stage-3a-brief-06.md`): no close is recorded; a held contract's price is read from its chain when shown. The server calls it when due from 3c. A contract a corporate event may have adjusted is found only by its OCC symbol, which 3b's Wealthsimple rows state; until then such a contract's close is refused, not guessed.
- **TMX quotes' time** is when TMX served the quote. 3c shows a price's age from it and never calls it a trade time.
- **Company ex-dates before 2024-05-28** are dated on XIC's sessions (fixed 2026-09-24; see "Taken" above).
- **Fidelity's paying funds** (fixed 2026-09-24). Its history service lists one date per distribution, names none of them the ex-date and leaves out reinvested distributions, so it is no longer read. Fidelity's own releases on newswire.ca state them: each month's cash distributions (a table per record and pay date, the ex-date by the exchange's rule) and each year's final reinvested capital gains (the ex-date stated); estimates and a terminating fund's special distributions are not read. One read takes every such release from 400 days back, paging the organization's list until it passes that day.
