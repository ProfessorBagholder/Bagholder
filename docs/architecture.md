...
2. **Layered model cache**, single flight. *Done*: each input is read again only when its counter moved and each layer rebuilt only when an input it reads did, so a quote tick reads the quotes and re-marks the positions and the match is not run again (tests hold the layers pointer-equal, and the layered base equal to one built from scratch). A built view is shared per base, filter set and open trade, so the same data asked for again costs nothing. On a real book: a tick 368 ms → 40 ms, the same model asked again 111 ms → 7 ms. *Left for stage 5, on purpose:* splitting the view itself into a closed half and a live half, so a tick builds only the live half. In today's view the two are interleaved through untyped JSON (options, cashflow and portfolio each read both); it is rewritten as typed structs in stage 5 and is split there, once, rather than twice.
