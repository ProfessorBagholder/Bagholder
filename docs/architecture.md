...
- **Guards against Bagholder's own automation misbehaving.** The bracket engine's orders per bracket per minute are capped: a bracket that trips the cap is stopped and the person told, because that many orders in a minute can only be a fault. A trigger fires only on the broker's own quote, and a quote that is not current by its own time is a failed read, shown, never acted on. None of this limits what the person, or an agent they have authorised, chooses to do; any limits on size or count are theirs to set, or not.
