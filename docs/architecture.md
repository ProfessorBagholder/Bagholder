...
- **A fill is booked once, however it is read.** Each execution the broker reports is booked in the book as a provisional record (§6), identified by the broker's own execution id or, where the broker gives none, by the order and the quantity newly filled since the last booking; the booking and the order's event are written in one transaction. Reading the same fill again, two read-backs at the same moment, or an order filling in several parts each book exactly what is new.
- **Nothing that follows live orders works from a partial list.** Open orders and live brackets are found by their state, whatever their number and however many orders came before them; no list the engine acts on is cut at a count.
...
- **Tested against a fake broker that misbehaves**: a reply lost or unreadable after a send (the order must end up read back, never "failed"), an order filling in several parts (the book must hold exactly the filled quantity), the same fill read twice and two read-backs at once (booked once), a history of thousands of orders with live ones among the oldest (all followed), a duplicate fill, a cancel confirmed after a fill, replies out of order, a session lapsing mid-swap. Every state and every move has a test.
...
## 13. Data flow: from a change in the world to the screen

One rule, end to end: **work is done because something changed or something is needed, only that work is done, and only what it changed reaches the screen.** Nothing is reloaded, recomputed or redrawn to be safe, nothing is loaded before it is needed, and nothing already loaded is loaded again.

- **Sources are read on demand.** A source is read because something needs it: a screen showing what it feeds, a notification the person turned on, an agent's request, or a moment known in advance (a token's expiry, the Bank of Canada's 16:30 rate, a pull time). While an order is open or a bracket is live, the broker is read on the cadence the order needs, whether or not anyone is looking. A read that finds nothing new changes nothing. Every periodic read that remains is listed with its reason; a test counts them and fails on one not listed.
- **The engine recomputes only what a change touches.** A change to the record re-derives the book's figures in full, which takes milliseconds on a real book. A price updates only the positions of that instrument and the totals that include them. What moved is found by comparing new figures with the previous ones by entity id and field. A test holds the result equal to a full rebuild for every kind of change.
- **The page loads what is on screen, when it is first needed.** It subscribes to the data of what it shows: the header, the current tab's cards under the current filters, an open instrument. A tab never visited loads nothing. A tab already visited stays loaded while the app is open, so going back to it is instant. Long lists (trades, news, distribution history, orders) send the rows in view and more as the person scrolls.
...
- **Held by tests.**
  ...
  3. For every kind of change, and for switching tabs and changing filters, a test watches the page and fails if an element that did not change is touched.
...
- **The tools are the interface's own operations (§12), through one adapter.** The MCP server is built into Bagholder and speaks to the running app; its tools are generated from the interface's declared operations, each with a plain description, so a capability the app gains reaches agents by declaring it once, with no agent-specific code. The separate filings MCP server is folded into this one. Broker credentials and sessions are never visible to an agent.
- **Whether an agent trades, and how, is the person's choice** (`docs/decisions.md`). Execution treats an agent's orders exactly as the person's own: the same state machines, the same broker confirmation, the same record of who asked.
- **What comes from outside is marked as such.** News headlines, filing text and anything else a source wrote reaches the agent labelled as outside content, so text planted in a headline cannot pass for an instruction.
- **An agent's request is demand like any other** (§13): it can ask for fresh data, within the same pacing every source keeps.
...
- Trade marks (§2 Trade): `SPEC.md` defines only `reward`. The engine also records how a round trip came to be (`deposited`, `split`, `continued`, `from-event`, `transferred`, `rolled`, `assignment`, `no-expiry-record`). Each is defined in `SPEC.md` at the switch, or none reaches the page. `basis-unknown` is a gap on the figure, not a mark: at the switch the page reads it from the figure's gaps (`web/src/lib/Trades.svelte` looks for it among the marks today).
