...
| Two stores, migrations, backups (§6, §17) | Change: split the file; numbered migrations, snapshots, tests on real old databases; automatic backups | medium |
...
| Change detection (§8, §12) | Change: engine reports what moved; stream carries numbered, typed changes | large |
...
| What the page loads and keeps (§13) | Change: today every open loads the whole model (about 1 MB on a real book: every trade, a thousand news items) whatever tab is shown, nothing is kept between opens but a few display preferences, and no read can be answered "unchanged". Needed: subscriptions per screen, paged lists, the browser-side store with versions, resume by version, conditional on-demand reads, and the request-budget and second-open tests | large |
...
| Failures shown (§16) | Change: per-source health on the header and cards; every discarded error dealt with; the page says when the app cannot start | medium |
| Updater and running as a service (§17) | Change: health by answer not by uptime, book snapshot and restore, signed releases, service install | medium |
| Periodic work (§15) | Change: the list of remaining timers and the test that counts them | small |
...
5. **Interface and running**: what the page loads and keeps (subscriptions per screen, kept between opens, resume by version), access layer, AI agents through one generated MCP server, typed numbered stream, failures shown, safe updater, service install, timer list.
