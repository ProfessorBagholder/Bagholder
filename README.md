...
- **Trades**: every closed trade with its executions, a price path across the fills, and a journal with a thesis, a grade and tags. Each one also carries the short interest in the instrument, from the regulator that publishes it: the shares reported still sold short and what they changed from, days to cover against that market's own daily volume, and the part of trading that was sold short, which the US reports every trading day. The same figures head the News card once you search a symbol, so any ticker carries them beside its news. FINRA for a US listing, CIRO for a Canadian one, so a TSX-V or CSE name is covered like any other. Shares, options and crypto are all matched FIFO per account; covered calls, rolls, expiries and assignments are handled.
