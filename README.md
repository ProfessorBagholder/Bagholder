...
- **Notifications**: the system's own, through the browser, for what happened while you were not looking: an order filled, partly filled, rejected, expired or cancelled at Wealthsimple; a stop or target that filled; a bracket that ended on its own or a leg Wealthsimple refused; the session expiring; a sync that keeps failing; a new release. Each kind is a switch in the menu, off until you turn it on, and the browser asks once whether the site may notify you. They arrive while Bagholder is open in a tab of any browser on any platform, and clicking one opens the Orders panel on the tab that carries it.
