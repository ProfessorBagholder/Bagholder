...
- **Trades**: every closed trade with its executions, a price path across the fills, and a journal with a thesis, a grade and tags. Each one also carries the short interest in the instrument, from the regulator that publishes it: the shares reported still sold short and what they changed from, how much of the free float that is, days to cover against that market's own daily volume, the part of trading that was sold short, which the US reports every trading day, and the short interest at each of the last reports as a line. The same figures head the News card once you search a symbol, so any ticker carries them beside its news, and the Markets tab ranks everything you hold and watch by how much of it is sold short. FINRA for a US listing, CIRO for a Canadian one, so a TSX-V or CSE name is covered like any other. Shares, options and crypto are all matched FIFO per account; covered calls, rolls, expiries and assignments are handled.
...
- **Markets**: a sector heatmap of your holdings, your watchlist, the S&P/TSX 60, the hundred largest US companies or the largest foreign companies listed in the US, sized by value and coloured by the day's change; a watchlist of listings, indices, futures (the S&P 500, Nasdaq 100, Dow and Russell E-minis for the read after hours, and the commodities), rates and currency pairs with live prices and day changes, added from the same search as everything else; the heatmap on its own at an address you can bookmark for a wall display or a second monitor, cycling through scopes like a dashboard if you ask it to, the disclosures of every ticker you hold, watch or have seen a switch away in the News card, with a search that finds any ticker's, and, in one card, the news on every symbol you hold or watch, with the companies' own press releases kept apart from the stories about them, from TMX Money and Nasdaq, searchable and scoped to a symbol with a click.
...
- **Notifications**: the system's own, under Bagholder's name and icon, for what happened while you were not looking: an order filled, partly filled, rejected, expired or cancelled at Wealthsimple; a stop or target that filled; a bracket that ended on its own or a leg Wealthsimple refused; the session expiring; a sync that keeps failing; a new version; a press release on the tickers you choose, from the wires or, where no wire carried it, the issuer's own filing; a new disclosure on the tickers you choose: your holdings, your watchlist or every ticker the book has seen, each set its own switch. Each kind is a switch in the menu, off until you turn it on. On a Mac, Windows or Linux desktop the app posts them itself, and the system asks once whether Bagholder may notify you; in the container the page shows them through the browser while it is open. A bell in the header keeps the history, with a badge for what you have not looked at.
