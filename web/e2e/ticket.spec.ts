...
// holds NVDA and AAPL as open positions in the USD margin account ("Trading"), TD as an
// open position in the CAD "TFSA", and UBER as a fully-closed trade in "Trading" with no
// open position -- real book data, not synthesised, so a Buy/Sell icon's availability
// (SPEC: Sell only where a position is held) is the book's own truth.
//
...
// falls back to the only margin account, "Trading" (acct-trading). Those rules let the
// test compute the exact doc key up front.
...
test('opens from a ⌘K listing row\'s Buy icon, on the account holding the symbol', async ({ page, request }) => {
  ...
  await expect(page.locator('#tk-account')).toHaveValue('acct-trading') // Trading holds the NVDA position
})
...
test('opens from a ⌘K listing row\'s Sell icon, only offered where a position is held', async ({ page, request }) => {
  ...
  await expect(page.locator('#tk-qty')).toHaveValue('25') // the held quantity, not the Buy default of 1
...
test('only the order types the quote reports for the security are offered', async ({ page, request }) => {
  await openWithStatus(page, request, {}, '', () => {}, {
    [quoteKey('TD', 'sec-td', 'acct-tfsa', 'TSX')]: { ok: true, orderTypes: ['MARKET', 'LIMIT'] },
  })
  await ready(page)
  await page.keyboard.press('Control+k')
...
test('Max sets the whole shares the account\'s buying power covers, and is off when unknown', async ({ page, request }) => {
  await openWithStatus(page, request, {}, '', () => {}, {
    [quoteKey('NVDA', 'sec-nvda', 'acct-trading', 'NASDAQ')]: { ok: true, buyingPower: 1000 },
  })
  await ready(page)
  await page.keyboard.press('Control+k')
  await page.getByRole('textbox', { name: 'Search' }).fill('NVDA')
...
test('a stop loss and target round to the cent even off a quote with extra precision', async ({ page, request }) => {
  await openWithStatus(page, request, {}, '', () => {}, {
    [quoteKey('AAPL', 'sec-aapl', 'acct-trading', 'NASDAQ')]: { ok: true, quote: { last: 163.47, currency: 'USD', multiplier: 1 } },
  })
  await ready(page)
  await page.keyboard.press('Control+k')
...
test('a trailing stop\'s loss is its distance exactly, and it reads "Starts at"', async ({ page, request }) => {
  await openWithStatus(page, request, {}, '', () => {}, {
    [quoteKey('NVDA', 'sec-nvda', 'acct-trading', 'NASDAQ')]: { ok: true, quote: { last: 165.4, currency: 'USD', multiplier: 1 } },
  })
  await ready(page)
  await page.keyboard.press('Control+k')
  await page.getByRole('textbox', { name: 'Search' }).fill('NVDA')
...
test('a quote that cannot be had says why, and the figures are dashes', async ({ page, request }) => {
  await openWithStatus(page, request, {}, '', () => {}, {
    [quoteKey('UBER', 'sec-uber', 'acct-trading', 'NYSE')]: { ok: false, error: 'Wealthsimple has no quote for UBER.' },
  })
  await ready(page)
  await page.keyboard.press('Control+k')
  await page.getByRole('textbox', { name: 'Search' }).fill('UBER')
...
test('the review step shows the order, the risk, and Available margin after on a margin account', async ({ page, request }) => {
  await openWithStatus(page, request, {}, '', () => {}, {
    [quoteKey('NVDA', 'sec-nvda', 'acct-trading', 'NASDAQ')]: {
      ok: true,
  ...
  await expect(rows.filter({ hasText: 'Position size' })).toContainText('1.8% of net asset value')
...
test('submitting sends Wealthsimple\'s own request shape and flashes the order placed', async ({ page, request }) => {
  await openWithStatus(page, request, {}, '', () => {}, {
    [quoteKey('NVDA', 'sec-nvda', 'acct-trading', 'NASDAQ')]: { ok: true, quote: { last: 165.4, currency: 'USD', multiplier: 1, securityId: 'sec-nvda' } },
  })
  await ready(page)
  ...
  await expect.poll(() => sent).toEqual({
    symbol: 'NVDA', securityId: 'sec-nvda', accountId: 'acct-trading', side: 'BUY', type: 'LIMIT', tif: 'DAY',
