...
// a real payload: the quotes scenario the server's own wire snapshots pin
import scenario from '../../../tests/wire/scenario_quotes.json'
...
function figuresOnlyOthersHave(model: Model, one: Model['positions'][number]): Set<number> {
  const of = (p: Model['positions'][number]) => [p.last, p.mv, p.unreal, p.avg, p.cost].filter((v): v is number => typeof v === 'number' && v !== 0)
  const own = new Set(of(one))
  return new Set(model.positions.filter((p) => p !== one).flatMap(of).filter((v) => !own.has(v)))
}

function book(): Model {
  return structuredClone(scenario.wire) as unknown as Model
}
...
describe('one holding\'s price moves', () => {
  it('touches that holding\'s cells and the totals, and nothing else', () => {
    ...
    const veqt = model.positions.find((p) => p.symbol === 'VEQT')!
    const rows = [...container.querySelectorAll('tbody tr')] as HTMLElement[]
    const veqtRow = rows.find((r) => r.textContent!.includes('VEQT'))!
    ...
    applyOps(model, [
      ...
      ['set', ['positions', row, 'unreal'], 1334],
      ['set', ['positions', row, 'unrealPct'], 0.2681],
      ['set', ['portfolio', 'marketValue'], model.portfolio.marketValue + 70],
      ['set', ['portfolio', 'unrealized'], model.portfolio.unrealized + 70],
    ] as Op[])
    flushSync()
    const seen = stop()
    ...
    expect(formatted.filter((v) => otherFigures.has(v as number))).toEqual([])
  })
  ...
  it('the old way, for contrast: replacing the list re-runs every holding for the same change', () => {
    ...
    formatted.length = 0
    const next = book()
    next.positions.find((p) => p.symbol === 'VEQT')!.last = 63.1
    model.positions = next.positions // a fresh list of fresh objects, as a whole-model reload gives
    flushSync()
    expect(formatted.filter((v) => otherFigures.has(v as number)).length).toBeGreaterThan(0)
  })
