...
struct EquityCurveCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            if let points, !points.isEmpty {
                LiveEquityCurveDrawing(points: points, hairlineX: selectedIndex == nil ? nil : selectedX, selectedIndex: selectedIndex)
                    ...
                    .overlay(alignment: .topLeading) {
                        if let tipText, let pt = selectedPoint {
                            ChartTipOverlay(text: tipText, hairlineX: pt.x, anchorY: pt.y, chartSize: chartSize)
                        }
                    }
    ...
    private var selectedPoint: CGPoint? {
        guard let points, let i = selectedIndex, chartSize.width > 0, chartSize.height > 0 else { return nil }
        let vals = points.map(\.equity)
        let lo = vals.min() ?? 0
        let hi = vals.max() ?? 1
        let span: Double = (hi - lo) == 0 ? 1.0 : (hi - lo)
        let n = CGFloat(max(points.count - 1, 1))
        let x = CGFloat(i) / n * chartSize.width
        let y = chartSize.height - CGFloat((vals[i] - lo) / span) * (chartSize.height - 4) - 2
...
struct MonthlyPnLCard: View {
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            if let bars, !bars.isEmpty {
                LiveMonthlyPnLDrawing(bars: bars, selectedIndex: selectedIndex)
                    ...
                    .overlay(alignment: .topLeading) {
                        if let tipText {
                            ChartTipOverlay(text: tipText, hairlineX: selectedCenterX, anchorY: selectedBarTopY, chartSize: chartSize)
                        }
                    }
    ...
    private var selectedBarTopY: CGFloat {
        guard let i = selectedIndex, chartSize.height > 0 else { return 0 }
        let sy = chartSize.height / 72
...
private struct ChartTipSizeKey: PreferenceKey {
    static var defaultValue: CGSize = .zero
    static func reduce(value: inout CGSize, nextValue: () -> CGSize) {
        value = nextValue()
    }
...
private struct ChartTipCapsule: View {
    ...
    var body: some View {
        Text(text)
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 8, style: .continuous)
                    .strokeBorder(Color(white: 0.86), lineWidth: 0.5)
            )
            .fixedSize()
    }
}

private struct ChartTipOverlay: View {
    let text: String
    let hairlineX: CGFloat
    let anchorY: CGFloat
    let chartSize: CGSize
    @State private var tipSize: CGSize = .zero

    var body: some View {
        ChartTipCapsule(text: text)
            .background(
                GeometryReader { geo in
                    Color.clear.preference(key: ChartTipSizeKey.self, value: geo.size)
                }
            )
            .onPreferenceChange(ChartTipSizeKey.self) { tipSize = $0 }
            .offset(x: tipX, y: tipY)
    }

    private var tipX: CGFloat {
        let w = tipSize.width
        let raw = hairlineX - w / 2
        guard chartSize.width > 0, w > 0 else { return raw }
        return min(max(raw, 0), max(chartSize.width - w, 0))
    }

    private var tipY: CGFloat {
        let h = tipSize.height
        let above = anchorY - 8 - h
        if above >= 0 {
            return above
        }
        return min(anchorY + 8, max(chartSize.height - h, 0))
    }
}
    ...
    var body: some View {
        Canvas { context, size in
            ...
            if let hx = hairlineX {
                ...
                context.stroke(hair, with: .color(Color(red: 174 / 255, green: 174 / 255, blue: 178 / 255)), lineWidth: 1)
            }
