...
- The page has no automated tests, so render it. Run a second instance on a copy of the user's data, never on the live database:
  ...
  ```
  (the binary finds the page by looking up from its own folder to the repository root). Each build gets a scratch home of its own, as each build keeps its own data folder.
